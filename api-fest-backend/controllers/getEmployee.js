...
const getEmployee = (req,res) => {
    try{
        ...
        Users.findOne({email: email}, (err,user)=>{
            // console.log(user);
            if(err) res.statusCode(400).json({statusCode: 200, message: "User not found! Please register!"});
            if(user.isLoggedIn===false) res.statusCode(400).json({statusCode: 200, message: "User not Logged in!"});
            else{
        ...
        return res.statusCode(err.statusCode | 400).json({statusCode: 400, message: err.message});
    }
}
